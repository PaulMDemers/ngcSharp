...
Each `emulator-summary.json` also includes a `timings` section that splits wall-clock time into emulation, measured diagnostics, GX frame/copy/coverage/TEV/texture dumps, memory dumps, and profile output. GX frame dumps also include nested renderer timings for FIFO expansion, replay, vertex decode, rasterization, EFB copies, and PNG writing. The suite-level `summary.csv` / `summary.json` surface the main timing columns as well; use those before starting an optimization pass so the next hotspot is visible in the artifact instead of inferred from console timing.
