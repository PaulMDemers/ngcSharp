...
- Inspect the new Sonic overlay/profile window around `0x80BC82B0` plus the paired-single transform loop around `0x8011DAD8..0x8011DAEC`; the old `0x800E3134` decision block now fires only once in the final late profiling slice, so it is no longer the wall to optimize.
