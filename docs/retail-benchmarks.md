...
- Added EFB copy sub-counters, mode signatures, a no-scale filtered display-copy fast path, an exact Sonic display-filter path, and a bulk I4 texture-copy path. A `sonic-20m` probe stayed frame-hash identical while dropping `gxFrameEfbCopyMs` from roughly 15.9s to 3.15s; the new summary shows Sonic's copy load is 162 filtered display copies using filter `8/8/10/12/10/8/8`, 322 I4 texture copies, and 484 clears.
