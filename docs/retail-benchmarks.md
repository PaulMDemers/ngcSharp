...
- A deep Sonic 20M GX probe reached 296 display copies and 589 texture copies before the 360s watchdog. It found two alternating XFB destinations (`0x004A2460` and `0x00538460`) with 97 nonblack copies each and 49 later black overwrites each; the auto frame currently preserves a partial SEGA logo from the last useful nonblack copy. A deep Pikmin 20M probe completed in 188.9s with 612 display copies at `0x003FE7A0`, 11 nonblack copies, and 600 later black overwrites; the auto frame preserves the Nintendo logo from the last useful nonblack copy.
