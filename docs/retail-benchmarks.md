...
- Inspect the new Sonic resource caller profile around `0x800E679C` / `0x800E68BC` and the still-dominant `0x800E3134` decision block before adding another exact fast-forward. The `0x800F135C` wrapper is now collapsed, so the next useful move is identifying whether those callers are polling missing subsystem progress or doing legitimate resource state transitions.
