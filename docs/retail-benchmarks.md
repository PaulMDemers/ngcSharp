...
- Added EFB copy sub-counters and a no-scale filtered display-copy fast path. A `sonic-20m` probe stayed frame-hash identical while dropping `gxFrameEfbCopyMs` from roughly 15.9s to 7.3s; the new summary shows Sonic's copy load is 162 filtered display copies, 322 texture copies, and 484 clears.
