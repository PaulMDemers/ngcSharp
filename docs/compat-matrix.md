...
- Optional `expected` milestones such as `stopReason`, final `pc`, `topPc`, `minTopPcCount`, `minPrsDecompressInstructions`, `minResourceLookupInstructions`, `minExternalInterruptLeafInstructions`, `minGxFifoBytes`, `minRenderedQuads`, `minRenderedTriangles`, `frameSource`, `frameSourceAddress`, `frameSourceCopyIndex`, `minDisplayCopies`, `minTextureCopies`, `minNonblackDisplayCopies`, `minMaxDisplayNonblack`, or `frameSha256`.
...
- Profile targets surface structured `topPc`, `topPcCount`, PRS decompression, resource-lookup, and external-interrupt leaf counters in `summary.csv`.
