...
- Focus the next Sonic resource investigation on the filtered top PCs, currently the `0x800F1438`/`0x800E2E34` task/resource staging path. The `0x800E2E20` helper uses a jump table into broader resource/task code, so it should be diagnosed with call-site profiling before any exact fast-forward is attempted.
