...
- Optional `expected` milestones such as `stopReason`, final `pc`, `topPc`, `minTopPcCount`, `minPrsDecompressInstructions`, `minResourceLookupInstructions`, `minGxFifoBytes`, `minRenderedQuads`, `minRenderedTriangles`, `frameSource`, `frameSourceAddress`, `frameSourceCopyIndex`, `minDisplayCopies`, `minTextureCopies`, `minNonblackDisplayCopies`, `minMaxDisplayNonblack`, or `frameSha256`.
...
- Profile targets surface structured `topPc`, `topPcCount`, PRS decompression, and resource-lookup counters in `summary.csv`.
